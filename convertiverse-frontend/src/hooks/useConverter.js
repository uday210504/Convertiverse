...
export const useConverter = () => {
  ...
  useEffect(() => {
    const fetchConverters = async () => {
      try {
        ...
        console.log('Available converters:', data);
        setConverters(data);

        // Get all input formats
        const formats = await getInputFormats();
        console.log('Available input formats:', formats);
  ...
  useEffect(() => {
    const fetchOutputFormats = async () => {
      ...
      try {
        console.log('Fetching output formats for:', selectedInputFormat);
        const formats = await getOutputFormats(selectedInputFormat);
        console.log('Available output formats:', formats);
        ...
        if (formats.length > 0) {
          ...
          console.warn('No output formats available for', selectedInputFormat);
        }
      } catch (error) {
